...
# podman build --build-arg="GIT_REV=$(git rev-parse HEAD)" .
...
FROM --platform=$BUILDPLATFORM docker.io/golang:${GO_VERSION}-alpine${ALPINE_VERSION} AS base
ARG GIT_REV
RUN echo $GIT_REV > /tmp/.gitrev
...
COPY --from=binary /registry /var/opt/registry
COPY cmd/entrypoint.sh /bin/registry
